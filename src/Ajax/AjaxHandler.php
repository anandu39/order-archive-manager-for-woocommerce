...
class AjaxHandler{
    ...
    */
    ...
    public function register_hooks(): void {
        add_action( 'wp_ajax_hw_woam_get_count', [ $this, 'handle_get_count' ] );
        add_action( 'wp_ajax_hw_woam_archive_batch', [ $this, 'handle_archive_batch' ] );
        add_action( 'wp_ajax_hw_woam_restore_batch', [ $this, 'handle_restore_batch' ] );
        add_action( 'wp_ajax_hw_woam_delete_batch', [ $this, 'handle_delete_batch' ] );
        add_action( 'wp_ajax_hw_woam_get_db_stats', [ $this, 'handle_get_db_stats' ] );
    ...
    */
    ...
    private function verify_request(): void {
        ...
        if ( ! current_user_can( 'manage_woocommerce' ) ) {
            wp_send_json_error(
                [ 'message' => __( 'You do not have permission to perform this action.', 'woo-order-archive-manager' ) ],
                403
            );
        }
    }
    ...
    */
    ...
    private function acquire_lock(): void {
        ...
        if ( get_transient( self::LOCK_KEY ) ) {
            wp_send_json_error(
                [ 'message' => __( 'Another archive job is already running. Please wait for it to finish.', 'woo-order-archive-manager' ) ],
                409
            );
        }
    ...
    public function handle_get_savings_estimate(): void {
        ...
        $order_count   = (int) $wpdb->get_var(
            $wpdb->prepare(
                ...
                array_merge( [ $before_date ], $statuses )
            )
        );
